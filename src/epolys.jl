...
# Not for users
function fast_Mtau(G::FiniteCoxeterGroup,i::Number,type_data::Any)
...
function fast_Stau(G::FiniteCoxeterGroup,n::Number,i::Number,type_data::Any)
...
function fast_qdtau(G::FiniteCoxeterGroup,i::Number,type_data::Any)
	# Returns q^(d(τ)) where τ = [L,ρ] is the ith GType
	return Pol{BigInt}(Pol(:q)^(type_data[i,:][2]))
end

function fast_Htau(G::FiniteCoxeterGroup,n::Number,i::Number,type_data::Any)
...
function EX(G::FiniteCoxeterGroup,g::Number,n::Number)
...
function EY(G::FiniteCoxeterGroup,g::Number,n::Number)
...
function fast_EX(G::FiniteCoxeterGroup,g::Number,n::Number,type_data::Any)
	# Returns the E-polynomial E(X;q) associated to the group G and a genus g surface with n punctures
	Z_size = Pol{BigInt}(orderpol(torus(rank(G)-semisimplerank(G))))
	T_size = Pol{BigInt}(orderpol(torus(rank(G))))
	type_sum = BigInt(0)
...
function fast_EY(G::FiniteCoxeterGroup,g::Number,n::Number,type_data::Any)
	# Returns the E-polynomial E(Y;q) associated to the group G and a genus g surface with n punctures
	Z_size = Pol{BigInt}(orderpol(torus(rank(G)-semisimplerank(G))))
	G_size = Pol{BigInt}(orderpol(G))
	g_size = Pol{BigInt}(Pol(:q)^(BigInt(degree(G_size))))
